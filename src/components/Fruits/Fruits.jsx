...
function Fruits() {
    ...
    // const handleRemoveFruit = (fruits) => {
    //   setCollectedFruits(collectedFruits.filter(f => f.id !== fruit.id))
    // }
      ...
      <>
        <ul>
            {fruits.map((fruit) => (
                ...
                <strong>{fruit.inStock ? "in Stock" : "Out of Stock"}
                </strong>
                <button onClick={() => toggleStock(fruit.id)}>{fruits.inStock ? "in Stock" : "Out of Stock"}</button>
                </li>
            ))
            }
        </ul>
